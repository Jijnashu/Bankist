...
const formatCur = function (value, country, currency) {
...
const displayDate = function (date, local) {
  ...
  const dayPassed = calcDay(new Date(), date);
  // console.log(dayPassed);
  if (dayPassed === 0) return 'Today';
  if (dayPassed === 1) return 'Yesterday';
  if (dayPassed <= 7) return 'Last Week';
  else {
...
const displayMovements = function (acc, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    const displayDateMov = displayDate(dateMov, acc.locale);
    const type = mov > 0 ? 'deposit' : 'withdrawal';
    const formatedNum = formatCur(mov, acc.locale, acc.currency);
    ...
    <div class="movements__date">${displayDateMov}</div>
...
const displaySummery = function (acc) {
  ...
  const depositedNum = formatCur(deposit, acc.locale, acc.currency);
  labelSumIn.textContent = `${depositedNum}`;
  const withdrew = Math.abs(
    acc.movements.filter(mov => mov < 0).reduce((acc, mov) => acc + mov, 0)
  );
  const withdrewNum = formatCur(withdrew, acc.locale, acc.currency);
  labelSumOut.textContent = `${withdrewNum}`;
  const interest = acc.movements
    .filter(mov => mov > 0)
    .map(mov => (mov * acc.interestRate) / 100)
    .filter(mov => mov > 1)
    .reduce((acc, mov) => acc + mov);
  const interestNum = formatCur(interest, acc.locale, acc.currency);
...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin == Number(inputLoginPin.value)) {
    ...
    //   i % 2 === 0 ? cur : (cur.style.backgroundColor = 'red');
    // });
...
// const calcDay = (date1, date2) =>
//   Math.round(Math.abs((date1 - date2) / (1000 * 60 * 60 * 24)));
// const day = calcDay(new Date(2023, 2, 15), new Date(2023, 2, 25));
// console.log(day);
